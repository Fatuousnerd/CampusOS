...
import { Search, Calendar, MapPin, Clock, Ticket, User, CheckCircle, ShieldAlert, Sparkles } from "lucide-react";
...
export default function EventsPage() {
    ...
    if (isRegistered) {
      ...
      if (event.price > 0) {
        if (balance < event.price) {
          toast.error(`Insufficient balance! This ticket costs KES ${event.price}.`);
          return;
        }
  ...
  const getCategoryColor = (cat: EventType["category"]) => {
    switch (cat) {
      case "Academic": return "text-indigo-500 border-indigo-500/20 bg-indigo-500/10";
      case "Party": return "text-pink-500 border-pink-500/20 bg-pink-500/10";
      case "Career": return "text-blue-500 border-blue-500/20 bg-blue-500/10";
      case "Sports": return "text-emerald-500 border-emerald-500/20 bg-emerald-500/10";
      case "Club": return "text-purple-500 border-purple-500/20 bg-purple-500/10";
    }
  };
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center justify-between border-b border-border pb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Book tickets to hackathons, welcome parties, sports finals, and career networking seminars.
          </p>
        </div>
      </div>
      ...
      {filteredEvents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-border rounded-2xl bg-card">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-sm">
            Try resetting categories or updating your search query to discover other campus schedules.
          </p>
        </div>
      ) : (
            ...
            return (
              <Card 
                ...
                className={`flex flex-col justify-between hover:shadow-md transition-all duration-300 bg-card border-border rounded-2xl overflow-hidden h-[420px] relative ${
                  isRegistered ? "ring-2 ring-primary/40 bg-primary/[0.01]" : ""
                }`}
              >
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-center">
                    <Badge variant="outline" className={`capitalize font-bold text-[10px] ${getCategoryColor(event.category)}`}>
                      {event.category}
                    </Badge>
                    <span className="text-xs font-extrabold text-primary font-sans">{event.priceText}</span>
                  </div>
                ...
                <CardContent className="flex-1 pb-4 space-y-4">
                  ...
                  <div className="space-y-2.5 pt-2 text-xs border-t border-border/40 text-muted-foreground font-semibold">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground truncate">{event.date}</span>
                    </div>
                    <div className="flex items-center gap-2">
          ...
          {selectedEvent && (
            <div className="flex flex-col">
              ...
              <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-6 text-white text-center relative overflow-hidden">
                ...
                <span className="text-[10px] tracking-widest font-black uppercase text-indigo-200">Campus Pass Ticket</span>
                <DialogTitle className="text-xl font-black mt-1 leading-tight text-white">{selectedEvent.title}</DialogTitle>
                <div className="mt-2.5 inline-flex items-center gap-1 bg-white/15 backdrop-blur-md px-2.5 py-0.5 rounded-full border border-white/20 text-[9px] font-bold">
                  <CheckCircle className="h-3 w-3 text-green-400" /> RSVP REGISTERED
                </div>
              </div>
              ...
              <div className="p-6 bg-card space-y-5 text-xs text-foreground">
                <div className="grid grid-cols-2 gap-y-4 gap-x-2 text-left">
                  <div>
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider block">Attendee</span>
                    <span className="font-extrabold text-foreground text-sm">Alex Mercer</span>
                  </div>
                  <div>
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider block">Student ID</span>
                    <span className="font-mono text-foreground font-semibold">CS-2023-8941</span>
                  </div>
                  <div className="col-span-2">
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider block">Venue Coordinates</span>
                    <span className="font-semibold text-foreground flex items-center gap-1 mt-0.5">
                      <MapPin className="h-3.5 w-3.5 text-primary shrink-0" /> {selectedEvent.venue}
                    </span>
                  </div>
                  <div>
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider block">Date</span>
                    <span className="font-semibold text-foreground mt-0.5 flex items-center gap-1">
                      <Calendar className="h-3.5 w-3.5 text-primary shrink-0" /> {selectedEvent.date.split(", ")[1]}
                    </span>
                  </div>
                  <div>
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider block">Arrival Time</span>
                    <span className="font-semibold text-foreground mt-0.5 flex items-center gap-1">
                      <Clock className="h-3.5 w-3.5 text-primary shrink-0" /> {selectedEvent.time.split(" - ")[0]}
                    </span>
                  </div>
                </div>
