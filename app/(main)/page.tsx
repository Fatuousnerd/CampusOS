...
export default function Dashboard() {
  ...
  const handleTopUp = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setTopUpAmount("");
      toast.success(
        `Successfully recharged KES ${amt.toLocaleString()} to Campus Card!`,
      );
    }, 1200);
  };
  ...
  return (
    <div className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your digital identity, services, and campus life in one
            dashboard.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="relative group overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-0.5 border border-white/10 h-64 flex flex-col justify-between">
            ...
            <div className="flex items-center gap-4 my-2">
              ...
              <div>
                ...
                <code className="text-[10px] text-white/70 bg-black/20 px-1.5 py-0.5 rounded mt-1.5 inline-block font-mono">
                  ID: {studentId}
                </code>
              </div>
            </div>
            ...
            <div className="flex justify-between items-end border-t border-white/15 pt-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  <ArrowUpRight className="h-4 w-4 text-indigo-700 font-bold" />{" "}
                  Top Up
                </Button>
              </div>
            </div>
        ...
        <Card className="bg-card border-border shadow-sm flex flex-col justify-between">
            ...
            {activeOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center">
                <p className="text-xs text-muted-foreground">
                  No active orders or service requests.
                </p>
              </div>
            ) : (
              <div className="space-y-3.5">
                {activeOrders.slice(0, 2).map((order) => (
                  <div
                    key={order.id}
                    className="p-2.5 border border-border/80 rounded-xl bg-muted/30"
                  >
                    <div className="flex justify-between items-start">
                      <div>
          ...
          <CardFooter className="pt-0 border-t border-border mt-auto">
            <Link
              href="/services"
              className="w-full text-center text-xs text-primary font-bold hover:underline pt-3.5 block"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          <h3 className="text-lg font-bold text-foreground flex items-center gap-1.5">
            <TrendingUp className="h-5 w-5 text-indigo-500" /> Campus OS
            Applications
          </h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3 text-xs leading-normal border-b border-border/50 pb-2">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-[11px] mt-0.5">
                    Campus Hackathon 2026 is scheduled for tomorrow at 9 AM.
                    Total prize: KES 50,000!
                  </p>
                  <Link
                    href="/events"
              ...
              <div className="flex gap-3 text-xs leading-normal border-b border-border/50 pb-2">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-[11px] mt-0.5">
                    Student Alex listing: &quot;Intro to Algorithms&quot;
                    textbook is currently KES 450.
                  </p>
                  <Link
                    href="/marketplace"
                    className="text-[10px] text-primary font-bold hover:underline block mt-1"
              ...
              <div className="flex gap-3 text-xs leading-normal">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-[11px] mt-0.5">
                    You are currently RSVP&apos;d to {rsvps.length} event(s).
                  </p>
                </div>
              </div>
            </div>
        ...
        <DialogContent className="sm:max-w-xs bg-background border border-border text-center py-6 flex flex-col items-center">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Scan this QR code at campus dining halls, laundry hubs, or event
              check-ins to authenticate.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        open={isTopUpOpen}
        onOpenChange={(open) => !isProcessing && setIsTopUpOpen(open)}
      >
        <DialogContent className="sm:max-w-sm bg-background border border-border">
          <form onSubmit={handleTopUp} className="space-y-4">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-foreground">
                Recharge Campus Card
              </DialogTitle>
              <DialogDescription>
                Top up your digital balance instantly to pay for catering,
                marketplace items, or services.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              <Button
                type="submit"
                disabled={
                  isProcessing || !topUpAmount || parseFloat(topUpAmount) <= 0
                }
