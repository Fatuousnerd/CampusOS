      ...
      activeOrders.forEach((order) => {
        if (order.status !== "Completed" && order.status !== "Cancelled") {
          ...
          if (nextProgress >= 100) {
            ...
            toast.success(
              `Service Completed: ${order.serviceName} is now ready!`,
            );
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center justify-between border-b border-border pb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Request on-demand student laundry, order catered food, mock print
            files, or schedule suite cleaning.
          </p>
        </div>
      </div>
            ...
            <CardHeader className="border-b border-border pb-4 bg-muted/20">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-bold text-foreground flex items-center gap-1.5">
                  <Truck className="h-5 w-5 text-primary" /> Active Service
                  Tracker
                </CardTitle>
                <Badge
                  variant="outline"
                  className="text-[10px] bg-primary/10 border-primary/25 text-primary font-bold"
                >
                  LIVE GPS MOCK
                </Badge>
              </div>
              <CardDescription className="text-xs">
                Real-time tracking of active student service orders. Progress
                increments 15% every 5 seconds.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 space-y-4">
              {activeOrders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground text-xs p-4 border border-dashed border-border rounded-xl">
                  ...
                  <p className="mt-1">
                    Book a laundry, printing, or food package to start tracking
                    live progress.
                  </p>
                </div>
              ) : (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                  {activeOrders.map((order) => (
                    <div
                      key={order.id}
                      className="p-3 border border-border rounded-xl bg-muted/20 hover:bg-muted/30 transition-all space-y-3"
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-[10px] font-bold border px-2 py-0.5 flex items-center gap-1 ${getStatusBadgeClass(order.status)}`}
                        >
                      ...
                      <div className="flex justify-between items-center text-[10px] text-muted-foreground border-t border-border/50 pt-2 mt-1">
                        <span>
                          Paid: <strong>KES {order.price}</strong>
                        </span>
